<template>
  <div>
    ...
    <main class="max-w-7xl mx-auto p-6">
      <Breadcrumb :items="[{name: 'Home', to: '/'}, {name: 'Student', to: '/student/dashboard'}, {name: 'Calendar'}]" />
      ...
      <Modal :visible="showEventModal" @close="showEventModal = false">
        <div v-if="selectedEvent">
          ...
          <div class="mt-4 flex gap-2">
            <router-link 
              :to="`/student/event/${selectedEvent.id}`" 
...
export default {
  ...
  methods: {
    ...
    handleDateClick(date) {
      // Optional: Handle date clicks for creating events
